...
d1 = 0.05  # #\thickness of layer 1 cm

d2 = 2.0  # #\thickness of layer 2

tstep = 100.0*10**-15  # #\temporal distance between 2 measured points in data
time = []
ts22 = 0
...
for i in range(0, NFFT):
    if Freqs2[i] == 0:
        n1[i] = 3.125 - 1j*kgen(5.0, Freqs2[i+1])
        # n1[i] = nt[i] - 1j*kgen(alp[i+1], Freqs2[i+1])
        n2[i] = n_i
    else:
        n1[i] = 3.125 - 1j*kgen(5.0, Freqs2[i])
...
pl.plot(time2*10**12, E0s, label='E_s', color='k')
pl.plot(time2*10**12, E0p, label='E_p', color='r')
pl.xlabel('Time (ps)')
pl.legend()
pl.xlim(-5, 50)
pl.ylabel('Amplitude (a.u.)')
...
nn0 = int((100*10**-15)/(time2[1] - time2[0]))
if ((100*10**-15)/(time2[1] - time2[0])) - nn0 >= 0.5:
    nn0 += 1
