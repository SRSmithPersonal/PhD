...
class Worker2:
    ...
    def call2(self):
        ...
        for zk in range(0, self.ln):
            ...
            res = minimize(self.err1, self.initSet1, method='nelder-mead', options={'xtol': 1e-8, 'maxiter': 1000, 'disp': False})
...
class Worker1k:
    ...
    def set_files(self, es, ep):
        ...
        gg = gauss(np.linspace(0, len(self.time), len(self.time)), b=np.argmax(np.abs(self.YSS)), ccd=bread)
        gg2 = gauss(np.linspace(0, len(self.time), len(self.time)), b=np.argmax(np.abs(self.YSS - self.YSS * gg)), ccd=bread2)
...
class Worker2k:
    ...
    def h_theory1p(self, f, n1, n2n):  # /Returns the theoretical Transfer function ratio for a given frequency and complex
        # refractive index\
        p1 = self.attenuation1(n1, f) ** 2
    ...
    def h_theory1s(self, f, n1, n2n):  # /Returns the theoretical Transfer function ratio for a given frequency and complex
        # refractive index\
        p1 = self.attenuation1(n1, f) ** 2
...
def recur2k(f, S, P, Si, Pi, Si2, Pi2, n, k, l):
    if len(f) > 80:
        ...
        high = Process(target=recur2k, args=(f_high, S_high, P_high, S_highi, P_highi, S_highi2, P_highi2, n_high, k_high, l))
        low.start()
        high.start()
        low.join()
        high.join()
...
def start_d2(mc):
    ...
    # mc.m1.semilogy(freq_s * 10 ** -12, np.abs(y_p_freq2n), L='Ellipsometry', xL='Frequency (THz)', yL='Refractive Index')
    # mc.m1.semilogy(freq_s * 10 ** -12, np.abs(y_s_freq2n), L='Transmission', xl=[botLim, upLim])
    # mc.m1.plot(freq_s * 10 ** -12, np.unwrap(np.angle(y_p_freq2n)), L='Ellipsometry', xL='Frequency (THz)', yL='Refractive Index')
    ...
    mc.m4.clf()
    mc.m4.plot(freq_s * 10 ** -12, nres1, L='Ellipsometry', xL='Frequency (THz)', yL='Refractive Index')
    mc.m4.plot(freq_s * 10 ** -12, nres2, L='Transmission', xl=[botLim, upLim])
    #
    mc.m5.clf()
    mc.m5.plot(freq_s * 10 ** -12, kres1, L='Ellipsometry', xL='Frequency (THz)', yL='Extinction Coefficient')
    mc.m5.plot(freq_s * 10 ** -12, kres2, L='Transmission', xl=[botLim, upLim])
    mc.lbl1.hide()
    mc.btn2.show()
    global ResShow
    if not ResShow:
