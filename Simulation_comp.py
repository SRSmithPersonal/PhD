...
d1 = 0.05*10**-2  # #\thickness of layer 1 cm

d2 = 2.0  # #\thickness of layer 2

tstep = 10.0*10**-15  # #\temporal distance between 2 measured points in data
time = []
ts22 = 0
...
for i in range(0, NFFT):
    if (time2[i] >= -2.0*10**-12) and (time2[i] <= 2.35*10**-12):
...
for i in range(0, NFFT):
    if Freqs2[i] == 0:
        n1[i] = 3.125 - 1j*kgen(0.03, Freqs2[i+1])
        # n1[i] = nt[i] - 1j*kgen(alp[i+1], Freqs2[i+1])
        n2[i] = n_i
    else:
        n1[i] = 3.125 - 1j*kgen(0.03, Freqs2[i])
...
TG1 = np.asarray(TG)
AG11 = np.gradient(np.asarray(AG1))
AG21 = np.gradient(np.asarray(AG2))
A11 = np.max(np.abs(E0s))
A12 = np.max(np.abs(AG21))
A21 = np.max(np.abs(E0p))
A22 = np.max(np.abs(AG11))
pl.plot(time2*10**12 + (18.782-0.66)-10, E0s/A11, label='E_s Sim', color='k')
pl.plot(time2*10**12 + (18.982-0.66)-10, E0p/A21, label='E_p Sim', color='r')
# pl.plot(TG1*10**12 - 10, AG21/A12, label='E_s Mes', color='r')
pl.xlabel('Time (ps)')
pl.legend()
pl.xlim(0, 50)
pl.ylabel('Amplitude (a.u.)')
pl.show()
pl.plot(time2*10**12 + (18.982-0.66)-10, E0s/A11, label='E_s', color='k')
...
pl.ylabel('Amplitude (a.u.)')
pl.show()
pl.plot(TG1*10**12 - 10, AG21/A12, label='E_s Mes', color='k')
pl.xlabel('Time (ps)')
pl.ylabel('Amplitude (a.u.)')
pl.xlim(0, 50)
pl.show()
pl.plot(time2*10**12 + (18.982-0.66)-10, E0p/A21, label='E_p Sim', color='k')
# pl.plot(TG1*10**12 - 10, AG11/A22, label='E_p Mes', color='k')
pl.xlabel('Time (ps)')
# pl.legend()
pl.xlim(0, 50)
pl.ylabel('Amplitude (a.u.)')
...
nn0 = int((1*10**-15)/(time2[1] - time2[0]))
if ((1*10**-15)/(time2[1] - time2[0])) - nn0 >= 0.5:
    nn0 += 1
